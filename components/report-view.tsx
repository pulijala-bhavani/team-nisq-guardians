...
import type { Candidate,EvaluationSummary,Feedback } from "@/lib/types";

type Report={feedback:Feedback;evaluation?:EvaluationSummary;candidate:Candidate;sessionId:string;completedAt:number;duration:number;daysCovered:number[]};
function formatDuration(seconds:number){const minutes=Math.max(1,Math.round(seconds/60));return `${minutes} min`}
const dimensionLabels:Record<keyof EvaluationSummary["dimensions"],string>={technicalAccuracy:"Technical accuracy",technicalSpecificity:"Technical specificity",reasoning:"Engineering reasoning",communication:"Communication",productionAwareness:"Production awareness"};
...
export function ReportView(){
  ...
  const score=report.evaluation?.overallScore??Math.max(58,Math.min(94,72+report.feedback.strengths.length*5-report.feedback.gaps.length*2));
  return <>
    ...
    <section className="report-layout section-shell">
      ...
      <div className="report-content">
        ...
        {report.evaluation&&<article className="report-section app-card"><h2><span>◫</span>Evidence-based evaluation</h2><div className="evaluation-grid">{Object.entries(report.evaluation.dimensions).map(([key,value])=><div className="evaluation-row" key={key}><p><span>{dimensionLabels[key as keyof EvaluationSummary["dimensions"]]}</span><strong>{value}/100</strong></p><i><em style={{width:`${value}%`}}/></i></div>)}</div><p className="evaluation-note">Scored after all eight answers · {report.evaluation.engine==="gemini"?"Gemini semantic evaluation":report.evaluation.engine==="hybrid"?"Hybrid semantic and recovery evaluation":"Resilient fallback evaluation"}</p></article>}
